...
formEL.addEventListener('submit', (event) => {
  ...
  && Number.isNaN(inputEl[0].value) && Number.isNaN(Number(inputEl[1].value))) {
    const result = checkRegExp(inputEl[0].value, Number.isNaN(Number(inputEl[1].value)));
  ...
  if (inputEl[0].value === '' || Number.isNaN(Number(inputEl[1].value))) {
    call.innerText = 'Укажите Имя';
    inputEl[0].classList.add('fieldStyle');
  }
  if (inputEl[1].value === '' || Number.isNaN(Number(inputEl[1].value))) {
...
phoneEl.forEach((item, i) => {
  item.addEventListener('click', (event) => {
    event.preventDefault();
    if (phoneCheck[i].value === '') {
      phoneEl[i].innerText = 'Укажите телефон';
      phoneCheck[i].style.borderColor = 'red';
    } else if (Number.isNaN(Number(phoneCheck[i].value)) && checked[i].checked === false) {
      phoneEl[i].innerText = 'Подтвердите согласие на обработку данных';
    } else if (Number.isNaN(phoneCheck[i].value)) {
      phoneEl[i].innerText = 'Недопустимый телефон';
      phoneCheck[i].style.borderColor = 'red';
    } else if (phoneCheck[i].value !== '' && Number.isNaN(Number(phoneCheck[i].value)) && checked[i].checked === true) {
      accept.style.display = 'flex';
      substrate.style.display = 'block';
      setTimeout(() => {
        accept.style.display = 'none';
...
const mySound = new Audio('../sounds/click3.mp3');
const mySound2 = new Audio('../sounds/click1.mp3');
